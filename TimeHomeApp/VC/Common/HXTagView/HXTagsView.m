...
- (void)setTagAry:(NSArray *)tagAry isCancel:(BOOL)isCanCel delegate:(id)delegate {
    ...
    for (int i = 0; i < disposeAry.count; i++) {
        ...
        for (int j = 0; j < iTags.count; j++) {
            ...
            SmallTag *tagview = [self ittemButtonWithFrame:frame hasCancelButton:isCanCel];
...
//根据frame生成对应的标签按钮
- (SmallTag *)ittemButtonWithFrame:(CGRect)frame hasCancelButton:(BOOL)hasCancelButton {
    
    SmallTag *smallView = [[SmallTag alloc]init];
...
//    button.frame = frame;
    smallView.smallTagButton.layer.borderColor = _titleNormalColor.CGColor;
    ...
    [smallView.smallTagButton setTitleColor:_titleNormalColor forState:UIControlStateNormal];
