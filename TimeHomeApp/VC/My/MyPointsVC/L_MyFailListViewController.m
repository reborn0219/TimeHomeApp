...
@interface L_MyFailListViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation L_MyFailListViewController
...
- (void)httpRequestForGetGoodsLogList {
    ...
    [L_NewPointPresenters getGoodsLoglistWithState:@"-1" isexchange:0 updataViewBlock:^(id  _Nullable data, ResultCode resultCode) {
...
-(void)gotoExchangeInfoWithID:(NSString *)theid {
    ...
    //    webVc.url = [NSString stringWithFormat:@"http://shop.usnoon.com/mobile/index.php?app=member&act=login&token=%@&userid=%@&allow=1&ret_url/mobile/index.php?app=buyer_order&act=view&order_id=%@",appDlgt.userData.token,appDlgt.userData.userID,theid];
