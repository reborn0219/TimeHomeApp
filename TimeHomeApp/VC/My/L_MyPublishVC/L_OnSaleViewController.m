...
@interface L_OnSaleViewController () <UITableViewDelegate, UITableViewDataSource>
{
    NSInteger page;
}
...
@implementation L_OnSaleViewController
...
/**
 ...
 @param postid 帖子id
 */
- (void)httpRequestForOffGoodsWithModel:(BBSModel *)model {
    ...
    [L_NewMinePresenters offGoodsWithPostid:model.theid UpDataViewBlock:^(id  _Nullable data, ResultCode resultCode) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (resultCode == SucceedCode) {
                ...
                [self showToastMsg:@"下架成功" Duration:3.0];
                
//                [[NSNotificationCenter defaultCenter] postNotificationName:NOTICE_REFRESH_EDIT_DATA object:nil];

            }else {

                if ([data[@"errcode"] integerValue] == 90001 && [data[@"errmsg"] containsObject:@"该红包已领完"]) {
                    [_dataArray removeObject:model];
                    [self.tableView reloadData];
                    [self showToastMsg:@"下架成功" Duration:3.0];
            ...
            if (_dataArray.count == 0) {
                ...
                [self showNothingnessViewWithType:NoContentTypeData Msg:@"您没有在售的商品哦，赶快去集市发布商品吧！" eventCallBack:nil];
 ...
 */
- (void)httpRequestForGetOnSaledatalistRefresh:(BOOL)isRefresh {
    ...
    [BBSMainPresenters getPostList:@"-99" userid:@"" sortkey:@"new" contype:@"0" posttype:@"1,4" poststate:@"0" iscollect:@"0" isuserfollow:@"-1" tagsname:@"" pagesize:@"20" page:[NSString stringWithFormat:@"%ld",page] communityid:@"" updataViewBlock:^(id  _Nullable data, ResultCode resultCode) {
...
//    [self httpRequestForGetOnSaledatalistRefresh:YES];
//
//}
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshDataList) name:NOTICE_REFRESH_EDIT_DATA object:nil];
...
- (void)createTableView {
    ...
    _tableView.mj_header = [MJTimeHomeGifHeader headerWithRefreshingBlock:^{
        [selfWeak httpRequestForGetOnSaledatalistRefresh:YES];
        
    }];
    
    _tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        [selfWeak httpRequestForGetOnSaledatalistRefresh:NO];
    }];
            ...
            if (model.height - 14 > 0) {
                ...
                cell.threeButtonDidClickBlock = ^(NSInteger buttonIndex) {
                    ...
                    if (buttonIndex == 1) {
                        ...
                        [alertVc setEventCallBack:^(id _Nullable data,UIView *_Nullable view,NSInteger index)
                         {
                             if(index==ALERT_OK) {
                                 ...
                                 [self httpRequestForOffGoodsWithModel:model];
                                 
                             }
                             
                         }];
                        
                        [alertVc ShowAlert:self Title:@"提示" Msg:@"是否下架该帖子" oneBtn:@"取消" otherBtn:@"确定"];
                        ...
                        SCML.sourceid            = [NSString stringWithFormat:@"%@",model.theid];
                        ...
                        SCML.shareUrl            = [NSString stringWithFormat:@"%@",model.sharegotourl];
                        SCML.type                = 4;
