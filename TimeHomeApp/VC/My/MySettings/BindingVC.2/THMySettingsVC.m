...
- (void)setUpArrays {
    ...
    titles = [[NSArray alloc]initWithObjects:@"头        像",@"昵        称",@"个性签名",@"门  牌  号",@"真实姓名",@"性        别",@"生        日",@"标        签",@"我的地址",@"实名认证",@"绑定", nil];
    NSString *name = @"";
    name = [XYString IsNotNull:userData.name];

    NSString *sexString = @"其他";
    if ([userData.sex intValue] == 1) {
        sexString = @"男";
    }else if ([userData.sex intValue] == 2) {
        sexString = @"女";
    }
    NSString *signature = @"我在时代社区，你在哪？";
...
- (void)createTableView {
    _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-64 - 8) style:UITableViewStyleGrouped];
    ...
    [_tableView registerClass:[THBaseTableViewCell class] forCellReuseIdentifier:@"cellID"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 7) {
            ...
            THBaseTableViewCell *cell2 = [tableView dequeueReusableCellWithIdentifier:@"cellID"];
            ...
            cell2.rightLabel.text = userData.selftag;
            return cell2;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        switch (indexPath.row) {
            ...
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    THPicUploadViewController *picUploadVC = [[THPicUploadViewController alloc]init];
