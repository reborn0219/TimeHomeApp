...
@interface NewServiceDatePickerView ()
{
    NSString *titleMsg;
}
/**
...
@implementation NewServiceDatePickerView
...
- (void)viewDidLoad {
    ...
    if (![XYString isBlankString:_minDate]) {
        _datePicker.minimumDate = [XYString NSStringToDate:_minDate withFormat:@"YYYY-MM-dd HH:mm"];
    }
...
+ (NewServiceDatePickerView *)getInstance {
    NewServiceDatePickerView * givenPopVC = [[NewServiceDatePickerView alloc] initWithNibName:@"NewServiceDatePickerView" bundle:nil];
    return givenPopVC;
}
