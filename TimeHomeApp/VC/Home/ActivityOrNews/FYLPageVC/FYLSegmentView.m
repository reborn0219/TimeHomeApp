...
- (void)setupUI{
    ...
    //addview
    self.addView = [[UIView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH-60, 0,60,44)];
    [self.addView setBackgroundColor:kNewRedColor];
    UIButton * addBtn = [[UIButton alloc]initWithFrame:CGRectMake(0,-2,60,46)];
    ...
    //scrollView
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-60, 44)];
    
    self.scrollView.backgroundColor = kNewRedColor;
    self.scrollView.userInteractionEnabled = YES;
    self.scrollView.showsHorizontalScrollIndicator = NO;
    [self addSubview:self.scrollView];
    
    //top
    UIView *viewTop = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH-60, 44)];
    viewTop.backgroundColor = FYLColorFromHexadecimalRGB(0xf7f7f7);
  
    _itemWidth = (SCREEN_WIDTH-60)/_titles.count;
    ...
    //指示下标颜色
    _viewIndex.backgroundColor = [UIColor whiteColor];
    [viewTop addSubview:_viewIndex];

    //mask
    _maskLayer = [[CALayer alloc] init];
    _maskLayer.frame = CGRectMake(_itemWidth/2 - kMaskWidth/2,42, kMaskWidth, 2);
...
-(void)addBtnClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(FYLSegmentViewAddBtnClick)]) {
        [self.delegate FYLSegmentViewAddBtnClick];
    }
}
///标签点击事件
...
- (void)setFrameOfIndexView:(CGRect)frame{
    _maskLayer.frame = CGRectMake(frame.origin.x,42,frame.size.width, 2);
...
- (void)setScrollViewOffset:(CGRect)frame{
   ...
   if( _itemWidth * self.titles.count < (SCREEN_WIDTH - 60))
   {
       return;
   }
    ...
    if (rightMax >= pointOffset.x + (SCREEN_WIDTH-60) || rightMax >= self.titles.count * _itemWidth) {
        pointOffset.x = rightMax - (SCREEN_WIDTH-60);
