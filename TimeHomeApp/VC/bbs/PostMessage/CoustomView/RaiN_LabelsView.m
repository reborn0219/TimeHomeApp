...
@implementation RaiN_LabelsView
{
    RaiN_SmallLabels *smallView;
}
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _selectedBackgroundImage = [self imageWithColor:[UIColor whiteColor] size:CGSizeMake(1.0, 1.0)];
    }
    return self;
}
...
//设置标签数据和代理
- (void)setTagAry:(NSArray *)tagAry isCancel:(BOOL)isCanCel delegate:(id)delegate {
    ...
    for (int i = 0; i < disposeAry.count; i++) {
        ...
        for (int j = 0; j < iTags.count; j++) {
            ...
            RaiN_SmallLabels *tagview = [self ittemButtonWithFrame:frame hasCancelButton:isCanCel];
            [tagview.smallTagButton setTitle:tagTitle forState:UIControlStateNormal];
           
            if (index == _currentIndex) {
                
                tagview.smallTagButton.selected = YES;
                tagview.cancelButton.selected = YES;
            } else {
                tagview.smallTagButton.selected = NO;
                tagview.cancelButton.selected = NO;
            }
            tagview.smallTagButton.tag = index;
            tagview.cancelButton.tag = index;
...
//根据frame生成对应的标签按钮
- (RaiN_SmallLabels *)ittemButtonWithFrame:(CGRect)frame hasCancelButton:(BOOL)hasCancelButton {
    
    smallView = [[RaiN_SmallLabels alloc]init];
    smallView.frame = frame;
    ...
    smallView.smallTagButton.frame = CGRectMake(0, 10, w-10, h-10);
    //    button.frame = frame;
    smallView.smallTagButton.layer.borderColor = kNewRedColor.CGColor;
    [smallView.smallTagButton setTitleColor:kNewRedColor forState:UIControlStateNormal];
    smallView.smallTagButton.layer.borderWidth = _borderWidth;
    smallView.smallTagButton.layer.masksToBounds = _masksToBounds;
    smallView.smallTagButton.layer.cornerRadius = _cornerRadius;
    smallView.smallTagButton.titleLabel.font = [UIFont systemFontOfSize:_titleSize];
    [smallView.smallTagButton setTitleColor:_titleSelectedColor forState:UIControlStateSelected];
    [smallView.smallTagButton setBackgroundImage:_normalBackgroundImage forState:UIControlStateNormal];
    ...
    [smallView.smallTagButton addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    //    [view addSubview:button];
    
    if (hasCancelButton) {
        smallView.cancelButton.hidden = NO;
        smallView.cancelButton.frame = CGRectMake(w-20, 0, 20, 20);
        [smallView.cancelButton addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
        
    }else {
        
        smallView.cancelButton.hidden = YES;
    }
...
- (NSArray *)disposeTags:(NSArray *)ary {
    ...
    for (int i = 0; i < ary.count; i++) {
        THTags *tagsOne = ary[i];
        NSString *tagTitle = tagsOne.name;
        NSUInteger index = i;
        ...
        if (contentSize.width+_tagSpace > maxFrameWidth) {
            ...
            dict[@"buttonWith"] = [NSString stringWithFormat:@"%f",contentSize.width+_tagSpace+10];//标签的宽度
        }
