...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (CGRectContainsPoint(self.messageContentView.frame, touchPoint)) {
        if (self.messageDelegate && [self.messageDelegate respondsToSelector:@selector(XMVoiceMessageTapped:voiceStatus:)]) {
            [self.messageDelegate XMVoiceMessageTapped:self.message voiceStatus:self];
        }
    }
}
...
- (void)setup{
    ...
    [self.voiceSecondsLabel setHidden:NO];
}

- (void)updateConstraints{
    [super updateConstraints];

    CGFloat width = MIN((self.message.seconds.integerValue)*5+80, 190);
    ...
    }else if (self.message.ownerType.integerValue == XMMessageOwnerTypeOther) {
...
- (void)setMessage:(Gam_Chat *)message{
    ...
    [self.voiceSecondsLabel setText:[NSString stringWithFormat:@"%d''",message.seconds.integerValue]];
    
    if (message.ownerType.integerValue == XMMessageOwnerTypeOther) {
        
        [self.voiceReadStateImageView setHidden:!message.voiceUnRead.boolValue];
        [self.voiceStateImageView setImage:[UIImage imageNamed:@"message_voice_receiver_normal"]];
        
    }else if (message.ownerType.integerValue == XMMessageOwnerTypeSelf){
        
        [self.voiceStateImageView setImage:[UIImage imageNamed:@"message_voice_sender_normal"]];
        [_voiceReadStateImageView setHidden:YES];
    ...
    dispatch_source_set_event_handler(timer, ^{
            dispatch_sync(dispatch_get_main_queue(), ^{
                if (!wself.isVoicePlaying) {
                    ...
                    if (wself.message.ownerType.integerValue == XMMessageOwnerTypeSelf) {
                    ...
                    else if (wself.message.ownerType.integerValue == XMMessageOwnerTypeOther) {
...
- (UILabel *)voiceSecondsLabel{
    if (!_voiceSecondsLabel) {
        ...
        _voiceSecondsLabel.textColor = TITLE_TEXT_COLOR;
        _voiceSecondsLabel.font = [UIFont systemFontOfSize:10.0f];
    }
