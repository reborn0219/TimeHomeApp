...
@implementation BluetoothPresenter
...
//        if(1){
        if([scanBlueName isEqualToString:uukm.bluename]){
            ...
            if([uukm.version isEqualToString:@"3.0"])
            {
                ...
                ls_openKey =  uukm.openkey;//[self searchOpenKey:uukm.type :uukm.openkey];
...
//                    connectTimer =[NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(connectTimeout2) userInfo:nil repeats:YES];
...
//               NSString * s = [NSString stringWithFormat:@"蓝牙权限名字：%@----摇到的蓝牙名字：%@",uukm.name,peripheral.name];
...
//            UIAlertView * al = [[UIAlertView alloc]initWithTitle:@"22323" message:@"2323" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:@"cancel", nil];
//            [al show];
//            
//            NSData *dataCut;
//            dataCut = [data subdataWithRange:NSMakeRange(0,20)];
//
//            [_discoveredPeripheral writeValue:dataCut forCharacteristic:_characteristicWrite type:CBCharacteristicWriteWithResponse];
//
//            dataCut = [data subdataWithRange:NSMakeRange(20,data.length)];
//            [_discoveredPeripheral writeValue:dataCut forCharacteristic:_characteristicWrite type:CBCharacteristicWriteWithResponse];
...
-(void)setUp{
    ...
    //    [_customerService setCharacteristics:@[characteristicUUID]];
...
//        NSData * keyData = [ls_openKey dataUsingEncoding:NSUTF8StringEncoding];
//
//                [_peripheralManager startAdvertising:@{CBAdvertisementDataIsConnectable:@1,CBAdvertisementDataLocalNameKey:@"MyBLE",CBAdvertisementDataManufacturerDataKey:[NSData dataWithBytes:&newdata length:6],CBAdvertisementDataServiceUUIDsKey:@[ [CBUUID UUIDWithString:ServiceUUID]]}];
        
//        ls_openKey = @"11111111211S1pS4DztJ6";
        [_peripheralManager startAdvertising:@{CBAdvertisementDataLocalNameKey : ls_openKey}];

//        if (ls_openKey.length>18) {
//
//
//            [_peripheralManager startAdvertising:@{CBAdvertisementDataLocalNameKey : [ls_openKey substringToIndex:18]}];
//
//            [_peripheralManager startAdvertising:@{CBAdvertisementDataLocalNameKey : [ls_openKey substringFromIndex:18]}];
//
//        }else
//        {
//            [_peripheralManager startAdvertising:@{CBAdvertisementDataLocalNameKey : ls_openKey}];
