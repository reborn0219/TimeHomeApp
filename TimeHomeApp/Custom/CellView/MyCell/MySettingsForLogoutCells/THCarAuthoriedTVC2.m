...
@implementation THCarAuthoriedTVC2
...
- (void)setUp {
    ...
    _rightLabel.textColor = GREEN_COLOR;
...
- (void)setOwnerResidence:(OwnerResidence *)ownerResidence {
    ...
    NSString *string = @"";
    if (![XYString isBlankString:ownerResidence.communityname]) {
        string = [string stringByAppendingFormat:@"%@",ownerResidence.communityname];
    }
    if (![XYString isBlankString:ownerResidence.name]) {
        string = [string stringByAppendingFormat:@" %@",ownerResidence.name];
    }
    _leftLabel.text = string;
}
