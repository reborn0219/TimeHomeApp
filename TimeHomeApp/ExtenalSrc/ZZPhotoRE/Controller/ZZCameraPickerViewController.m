...
@interface ZZCameraPickerViewController()<UICollectionViewDelegate,UICollectionViewDataSource,ZZCameraFocusDelegate,ZZBrowserPickerDelegate,TOCropViewControllerDelegate>
...
//    _downView.alpha = 0.7;
    ...
    _takePhotoBtn = [[UIButton alloc]initWithFrame:CGRectMake(x, photoSize + 27, 50, 50)];
    [_takePhotoBtn setImage:TakePhoto_Btn_Pic forState:UIControlStateNormal];
    [_takePhotoBtn addTarget:self action:@selector(takePhoto) forControlEvents:UIControlEventTouchUpInside];
    [_downView addSubview:_takePhotoBtn];
...
-(void)takePhoto {
    ...
    if (self.cameraSourceType == ZZImageSourceForMutable) {
...
//         [UIImagePNGRepresentation(t_image) writeToFile:path atomically:YES];
         ...
         //+++++++++06.20++++++++++++增加
         if (self.cameraSourceType == ZZImageSourceForSingle) {
             
             if (self.isCanCustom) {
                 TOCropViewController *cropController = [[TOCropViewController alloc] initWithImage:t_image];
                 cropController.delegate = self;
                 if (self.imageAspectRatio != CameraImageAspectRatioSquare) {
                     
                     if (self.imageAspectRatio == CameraImageAspectRatioCustom) {
                         ...
                         cropController.defaultAspectRatio = (TOCropViewControllerAspectRatio)self.imageAspectRatio;
                     }
                 ...
                 cropController.rotateClockwiseButtonHidden = self.rotateClockwiseButtonHidden;
                 cropController.rotateButtonsHidden = self.rotateButtonsHidden;
                 cropController.aspectRatioLocked = self.aspectRatioLocked;
                 [self presentViewController:cropController animated:YES completion:nil];
             }else {
                 if (self.CameraResult) {
                     self.CameraResult(t_image);
                 }
                 [self dismissViewControllerAnimated:YES completion:nil];
             }
