...
//    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"LCActionSheet" ofType:@"bundle"];
    ...
    if (self.buttonTitles.count) {
        ...
        for (int i = 0; i < self.buttonTitles.count; i++) {
            ...
            [btn setTag:i];
            [btn setBackgroundColor:UIColorFromRGB(0xab2121)];
            ...
            if (i == self.redButtonIndex) {
                ...
                titleColor = LCColor(255, 255, 255);
...
//            UIImage *bgImage = [UIImage imageWithContentsOfFile:linePath];
            UIImage * bgImage=[self createImageWithColor:UIColorFromRGB(0x871c1c)];
            
            [btn setBackgroundImage:bgImage forState:UIControlStateHighlighted];
//            [btn set];
            [btn addTarget:self action:@selector(didClickBtn:) forControlEvents:UIControlEventTouchUpInside];
            
            CGFloat btnY = BUTTON_H * (i + (self.title ? 1 : 0))+20* i;
            [btn setFrame:CGRectMake(20, btnY, SCREEN_SIZE.width-40, BUTTON_H)];
...
//            [line setFrame:CGRectMake(0, lineY, SCREEN_SIZE.width, 20.0f)];
...
//    NSString *linePath = [bundlePath stringByAppendingPathComponent:@"bgImage_HL@2x.png"];
//    UIImage *bgImage = [UIImage imageWithContentsOfFile:linePath];
    UIImage * bgImage=[self createImageWithColor:UIColorFromRGB(0x595353)];
    
    // 取消按钮
    UIButton *cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [cancelBtn setTag:self.buttonTitles.count];
    [cancelBtn setBackgroundColor:UIColorFromRGB(0x8e8e8e)];
    ...
    CGFloat btnY = (BUTTON_H+20) * (self.buttonTitles.count + (self.title ? 1 : 0)) + 5.0f-20;
    [cancelBtn setFrame:CGRectMake(20, btnY, SCREEN_SIZE.width-40, BUTTON_H)];
    [bottomView addSubview:cancelBtn];
    
    CGFloat bottomH = (self.title ? BUTTON_H : 0) + (BUTTON_H+20) * self.buttonTitles.count + BUTTON_H + 5.0f+20;
