...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.borderColor = RGBCOLOR(200, 199, 204);
...
-(id)initWithPoint:(CGPoint)point titles:(NSArray *)titles images:(NSArray *)images fullWidth:(BOOL)fullWidth tableHeigth:(int)heigth
{
    self = [super init];
    if (self) {
        self.showPoint = point;
        self.titleArray = titles;
        self.imageArray = images;
        self.isFullScreenWidth=fullWidth;
        self.tableHeigth=heigth;
        self.frame = [self getViewFrame];
...
-(CGRect)getViewFrame
{
    ...
    if(self.tableHeigth>0)
    {
        frame.size.height=self.tableHeigth;
    }
    else{
        frame.size.height = ([self.titleArray count]>10? ([self.titleArray count]* ROW_HEIGHT/2):[self.titleArray count]* ROW_HEIGHT) + SPACE + kArrowHeight;
    }
    

    
    for (NSString *title in self.titleArray) {
        CGFloat width =  [title sizeWithFont:TITLE_FONT constrainedToSize:CGSizeMake(SCREEN_WIDTH-40, 100) lineBreakMode:NSLineBreakByCharWrapping].width;
//          CGFloat width =  [title sizeWithAttributes:@{NSFontAttributeName:TITLE_FONT}].width;
    ...
    if (self.isFullScreenWidth) {
        frame.origin.x=0;
        frame.size.width=SCREEN_WIDTH;
    }
    if ((frame.origin.y+frame.size.height)>SCREEN_HEIGHT) {
...
-(UITableView *)tableView
{
    ...
    rect.size.height =self.frame.size.height-(kArrowHeight + SPACE);
//    if (self.isFullScreenWidth) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor=UIColorFromRGB(0x818181);
